...
- データモデルの定義
  - `user_id = 1`はシステムユーザーとし、そのデータは全ユーザーが閲覧できる「デフォルトデータ」とする。
  - 一般ユーザー（`user_id >= 2`）が作成したデータは「カスタムデータ」とする。
  - 一般ユーザーは、デフォルトデータの編集・削除はできない。
  - 一般ユーザーは、自身が作成したカスタムデータのみ編集・削除できる。
...
- 特定のカテゴリに属するアイテム一覧の取得
  - 条件
    - カテゴリIDに紐づくアイテムを取得する。
    - デフォルトアイテムと、ログインユーザーが作成したカスタムアイテムを合算して取得する。
    - 削除フラグが`false`のレコードのみを対象とする。
    - 更新日が新しい順に表示する。
- アイテムをカテゴリに紐づけて新規作成（カスタムアイテム）
  - 条件
    - アイテム名は必須。
    - アイテム名は50文字以内。
    - 選択したカテゴリ内の全アイテム（デフォルトアイテムおよびカスタムアイテム）の中で、アイテム名が重複しない。
    - カテゴリの選択は必須。
    - 選択できるカテゴリは、デフォルトカテゴリまたは自身が作成したカスタムカテゴリであること。
    - 数量（`quantity`）を指定しない場合はデフォルト値として0を登録。
    - カテゴリ内でユーザーが作成できるカスタムアイテムは100件まで。
- アイテム情報（名前、在庫数）の更新（カスタムアイテム）
  - 条件
    - 編集対象は、自身が作成したカスタムアイテムのみとする。
    - アイテム名は必須。
    - アイテム名は50文字以内であること。
    - 選択したカテゴリ内の全アイテム（デフォルトアイテムおよびカスタムアイテム）の中で、アイテム名が重複しない。
...
- **概要：**　ユーザーが、特定のカテゴリに属する利用可能な全てのアイテム（デフォルト＋カスタム）の一覧を確認する。
- **アクター：**　ユーザー　
- **事前条件：**
  - ユーザーがシステムにログインしている。
...
- **成功シナリオ**　
  ...
  2. システムは、選択されたカテゴリに紐づく、デフォルトアイテムとログイン中のユーザーが作成したカスタムアイテムを全て取得する。 *1
  3. システムは、取得したアイテムの一覧を更新日の新しい順に表示する。
- **例外シナリオ**
  - （*1）表示可能なアイテムが1件もなかった場合（※基本的には発生しない想定）
    1. システムは、「表示可能なアイテムがありません」というメッセージを表示する。

### 【ユースケース6: アイテムを新規作成する】
- **概要：**　ユーザーが、特定のカテゴリに自分専用の新しいアイテムを追加する。
- **アクター：**　ユーザー　
- **事前条件：**
  - ユーザーがシステムにログインしている。 
...
- **成功シナリオ**　
  ...
  2. システムは、入力内容を検証する。（アイテム名：必須・文字数上限(50文字)・重複、カテゴリ：必須、数量：形式、登録上限数(カテゴリごとに100件)） *1 *2 *3 *4 *5
...
- **例外シナリオ**
  ...
  - （*5）カテゴリ内の登録アイテム数が上限の100件に達している場合
    1. システムは、ユーザーに「このカテゴリにはこれ以上アイテムを登録できません」というエラーメッセージを表示する。

### 【ユースケース7: アイテムを編集する】
- **概要：**　ユーザーが、自身で作成したカスタムアイテムの名前や数量を変更する。
- **アクター：**　ユーザー　
- **事前条件：**
  - ユーザーがシステムにログインしている。 
  - 編集対象のアイテムが存在し、ログイン中のユーザーが作成したカスタムアイテムである。
- **成功シナリオ**　
  1. ユーザーは、一覧から編集したいカスタムアイテムを選択し、新しい名前や数量を入力して、更新をシステムに要求する。 *1
  2. システムは、入力内容を検証する。（アイテム名：必須・文字数上限(50文字)・重複、数量：形式） *2 *3 *4 *5
  3. システムは、データベースの該当アイテム情報を更新する。 
  4. システムは、ユーザーに「アイテムの更新が完了しました」と通知する。
- **例外シナリオ**
  - （*1）編集対象がデフォルトアイテムだった場合
    1. システムは、ユーザーに「デフォルトアイテムは編集できません」というエラーメッセージを表示する。
  - （*2）アイテム名が空だった場合
    1. システムは、ユーザーに「アイテム名は必須です」というエラーメッセージを表示する。
  - （*3）アイテム名が50文字を超えている場合
    1. システムは、ユーザーに「アイテム名は50文字以内で入力してください」というエラーメッセージを表示する。
  - （*4）同一カテゴリ内に同じアイテム名が既に存在する場合
    1. システムは、ユーザーに「そのアイテム名は既に登録されています」というエラーメッセージを表示する。
  - （*5）数量が不正な形式（0未満の整数など）だった場合
    1. システムは、ユーザーに「数量は0以上の整数で入力してください」というエラーメッセージを表示する。

### 【ユースケース8: アイテムを削除する】
- **概要：**　ユーザーが自身で作成したカスタムアイテムを削除する。
- **アクター：**　ユーザー　
- **事前条件：**
  - ユーザーがシステムにログインしている。 
  - 削除対象のアイテムが存在し、ログイン中のユーザーが作成したカスタムアイテムである。
- **成功シナリオ**　
  1. ユーザーは、アイテム編集ページを開き、削除をシステムに要求する。 *1
  2. システムは、ユーザーに削除の最終確認を求める。
  3. ユーザーは、削除を承認する。 *2
  4. システムは、データベースの該当アイテムを論理削除する。
  5. システムは、ユーザーに「アイテムの削除が完了しました」と通知し、カテゴリ内のアイテム一覧ページに遷移する。
- **例外シナリオ**
  - （*1）削除対象がデフォルトアイテムだった場合
    1. システムは、ユーザーに「デフォルトアイテムは削除できません」というエラーメッセージを表示する。
